...
function parseForAcroformObject(pdfParser) {
  (catalogDict = pdfParser
    .queryDictionaryObject(pdfParser.getTrailer(), "Root")
    .toPDFDictionary()),
    (acroformDict = catalogDict.exists("AcroForm")
      ? pdfParser.queryDictionaryObject(catalogDict, "AcroForm")
      : null);
...
function parseOpts(pdfParser, fieldDictionary) {
  if (fieldDictionary.exists("Opts")) {
    var opts = pdfParser
      .queryDictionaryObject(fieldDictionary, "Opts")
      .toPDFArray()
      .toJSArray();
  ...
  switch (fieldType) {
    ...
    case "Tx": {
      ...
      if ((flags >> 25) & 1) {
        ...
        result["plainValue"] = parseTextFieldValue(
          pdfParser,
          fieldDictionary,
          "V"
        );
...
function parseField(
  ...
  var localFieldNameT = fieldDictionary.exists("T")
      ...
      : undefined,
    localFieldNameTU = fieldDictionary.exists("TU")
      ? toText(fieldDictionary.queryObject("TU"))
      : undefined,
    localFieldNameTM = fieldDictionary.exists("TM")
...
function PDFDigitalForm(pdfParser) {
  ...
  if (this.acroformDict) {
    var fieldsArray = this.acroformDict.exists("Fields")
      ...
      : null;
    if (fieldsArray)
      this.fields = parseFieldsArray(pdfParser, fieldsArray, {}, "");
  }
}
